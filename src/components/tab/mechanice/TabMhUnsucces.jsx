...
function TabMhUnsucces() {
  ...
  useEffect(() => {
    ...
    axios
      .post(
        `http://localhost:5000/api/repair-list-backlog-unsuccess`,
        {},
        {
          headers: {
  ...
  const updateRepairNotice = (id) => {
    navigate(`/mechanice-update-repair/${id}`);
  };
    ...
    <main>
      ...
      <div className='lg:hidden mt-10'>
        {data.map((repairData, idx) => {
          return (
            <div
              className='card rounded-sm bg-gray-200 shadow-xl mb-6 border-2'
              key={idx}
            >
